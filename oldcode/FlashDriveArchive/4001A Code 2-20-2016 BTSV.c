...
#pragma config(Motor,  port9,           Frontrightwheel, tmotorVex393_MC29, openLoop, reversed)
...
int flywheelspeed = 60;




int click_to_flywheel = 0;

task basicflywheel()//-+17
{
	int BL = (BackupBatteryLevel + nImmediateBatteryLevel);

	if(vexRT[Btn8U]==1)
	{
		motor(Leftflywheel) = 71 - BL;
		motor(Rightflywheel) = 71 - BL;
	}

	else if (vexRT[Btn8U]==1 && vexRT[Btn8R]==1)
	{
		motor(Leftflywheel) = 68 - BL;
		motor(Rightflywheel) = 68 - BL;

	}

	else if(vexRT[Btn8R]==1)
	{
		motor(Leftflywheel) = 65 - BL;
		motor(Rightflywheel) = 65 - BL;
	}

	else if(vexRT[Btn8L]==1)
	{
		motor(Leftflywheel) = 62 - BL;
		motor(Rightflywheel) = 62 - BL;
	}

	else if(vexRT[Btn8D]==1)
	{
		motor(Leftflywheel) = 47 - BL;
		motor(Rightflywheel) = 47 - BL;
	}
