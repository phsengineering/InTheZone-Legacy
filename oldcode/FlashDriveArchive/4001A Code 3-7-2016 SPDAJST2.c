#pragma config(Sensor, dgtl1,  Switch,         sensorTouch)
...
task autonomous()
{
	...
	wait(3.5);//Start flywheels up to speed wait for 1st ball
...
task ledees()
{
	turnLEDOff(green2); turnLEDOff(green3 );
...
task basicflywheel()
{
	...
	if(vexRT[Btn8R]==1)
	{
		...
		{
			/////////////button press gives a burst of speed//////////////
	if(SensorValue(Switch)==1)
	{
		Button=1;
	}

	if(SensorValue(Switch)==0)
	{
