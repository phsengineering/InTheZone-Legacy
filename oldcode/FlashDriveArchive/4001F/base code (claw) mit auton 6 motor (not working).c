...
task autonomous()
{
	...
	*/
	...
	motor[leftside]=-127;
	motor[rightside]=-127;
	wait(0.5); //unknown time same as above//
...
task usercontrol()
{
	...
	while(1==1)
	{
		...
		else if(vexRT[Btn6D]==0)
		{
			motor[arm]=0;
			motor[arm2]=0;
			motor[arm3]=0;///end claw goes up///
		}
		if(vexRT[Btn5D]==1) ///claw goes down///
		{
			motor[arm]=-127;
			motor[arm2]=-127;
			motor[arm3]=-127;
