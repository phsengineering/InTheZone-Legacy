...
task autonomous()
{
	startMotor(Leftflywheel, 60);
	startMotor(Rightflywheel, 60);
	wait(2.5);
...
//if the led int is between these numbers then do:
...
task basicflywheel()
{
	if(vexRT[Btn8U]==1)
	{
		...
		flywheelspeed=flywheelspeed+2;//increase speed by 2
		lednum=lednum+1;
	}
	else if(vexRT[Btn8D]==1)//decrease the speed by 2
	{
		while(vexRT[Btn8D]==1){}
		flywheelspeed=flywheelspeed-2;
		lednum=lednum-1;
	}
	else if(vexRT[Btn8L]==1)//go back to "home" speed of 62
	{
		while(vexRT[Btn8L]==1){}
		flywheelspeed=62;
		lednum=7;
	...
	else if(vexRT[Btn7U]==1)//increase the speed by 1
	{
		while(vexRT[Btn7U]==1){}
	...
	else if(vexRT[Btn7D]==1)//lower the speed by 1
	{
		while(vexRT[Btn7D]==1){}
		flywheelspeed=flywheelspeed-1;
		lednum=lednum-0.5;
...
task usercontrol()//main task runs and starts sub tasks which run simultaneously
{
	...
	while(1==1)
	{
		...
		{//if any of the flywheel buttons are being used then run the flywheel code
			startTask(basicflywheel);

			if(vexRT[Btn8R]==1)
			{
				motor(Leftflywheel) = flywheelspeed - 6;
				motor(Rightflywheel) = flywheelspeed - 6;
			}
		}
		...
		if(vexRT[Ch2]>0 || vexRT[Ch2]<0 || vexRT[Ch3]>0 || vexRT[Ch3]<0 || vexRT[Btn7L]==1 || vexRT[Btn7R]==1)
	...
	if(vexRT[Btn7L]==1) //turn a little to the left
		{
		...
		else if(vexRT[Btn7R]==1)//turn a little to the right
		{
			motor(Backleftwheel) = -30;
			motor(Frontleftwheel) = -30;
