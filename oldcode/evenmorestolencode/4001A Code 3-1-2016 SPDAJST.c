...
task autonomous()
{
	startMotor(Leftflywheel, 127);//Start flywheels up to speed
	startMotor(Rightflywheel, 127);//Start flywheels up to speed
	wait(3.75);//Start flywheels up to speed wait for 1st ball
	startMotor(Intake, 100);//Load first ball
	wait(.5);//Launch 1
	stopMotor(Intake);
	wait(3.5);//Wait for 2nd ball
	startMotor(Intake, 100);//2nd ball
	wait(.5);//Shoot 2nd ball
	stopMotor(Intake);
...
task usercontrol()//main task runs and starts sub tasks which run simultaneously
{
	...
	while(1==1)
	{
		...
		if(vexRT[Ch2]==0 && vexRT[Ch2]==0 && vexRT[Ch3]==0 && vexRT[Ch3]==0  || vexRT[Btn7R]==1 || vexRT[Btn7L]==1)
