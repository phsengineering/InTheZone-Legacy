...
task autonomous()
{
	if(SensorValue[Jumper1]==0) //run if jumper in port 1
	{
		...
		wait(1);
		motor(mobile)=0;
		while(SensorValue[Quad] < 720 || SensorValue[Quad2] < 720)
		{
			motor(leftside)=63;
		...
		motor(leftside)=0;
		motor(rightside)=0;
		while(SensorValue[Quad] < 800 || SensorValue[Quad2] < 800)
		{
		motor(leftside)=63;
			motor(rightside)=63; //drive to mobile
		}
	...
	//left side
	{
		if(SensorValue[Jumper2]==0) //run if jumper in port 2
		{
			...
			while(SensorValue[Quad] < 1905 || SensorValue[Quad2] > 1905)
			{
				motor(rightside)=127; //drive to mobile
			}
			startTask(Quadreset);
			motor(leftside)=0;
			motor(rightside)=0;
			...
			while(SensorValue[Quad] > -1905 || SensorValue[Quad2] > -1905)
			{
				...
				motor(bottom)=0;
				motor(fly)=0;
				motor(rightside)=-127; //back up with mobile
				motor(leftside)=-127;
			}
			startTask(Quadreset);
			while( SensorValue[Quad] > -480 || SensorValue[Quad2] < 480)
			{
				motor(leftside)=127; // complete turn towards drop zone
			}
			startTask(Quadreset);
			while(SensorValue[Quad] < 60 || SensorValue[Quad2] < 60)
			{
				motor(rightside)=127; //drive to zone
			}
			startTask(Quadreset);
			motor(leftside)=0;
			motor(rightside)=0;
