...
task autonomous()
{
	...
	*/
	...
	wait(1);
	motor(top)=50;//lift
	motor(bottom)=-50;//lift
	motor(leftside)=35;//forward
	motor(rightside)=35;//forward
	wait(1.25);
	motor(top)=0;
	motor(bottom)=0;//stop
	motor(leftside)=0;
	...
	wait(2);
	motor(top)=-25;
	motor(bottom)=25;//lower thing
	wait(.4);
	motor(fly)=-127;//release
	motor(top)=25;//raise top
	motor(bottom)=-25;//raise bottom
	wait(.6);
	motor(fly)=0;//stop fly
	motor(top)=0;//stop top
	motor(bottom)=0;//stop bottom
	motor(leftside)=-63;
	motor(rightside)=-63;
	wait(.75);
	motor(leftside)=0;
	...
	motor(top)=0;
	motor(bottom)=0;
	//god please help me and let this thing work so that we don't have to off ourselves
