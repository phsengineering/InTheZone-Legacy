...
if(SensorValue[Jumper1]==0) //run if jumper in port 1
	{
		...
		startTask(Quadreset);
		motor(leftside)=0;
		motor(rightside)=0;//stop drive dan and valmiki please
		motor(mobile)=127;//pick up mobile goal
		wait(.7);
		motor(mobile)=0;//stop mobile
		motor(top)=-30;
		motor(bottom)=-30;//drop lift off a cliff
		wait(.5);
		motor(top)=0;
		wait(1);
...
//	wait(5); for testing
		...
		while( SensorValue[Quad] > -480 || SensorValue[Quad2] < 480)
		{
			...
			motor(rightside)=63; // complete turn towards drop zone in the shape of an elephant
			motor(leftside)=-63;

		}
		startTask(Quadreset);
		while(SensorValue[Quad] < 300 || SensorValue[Quad2] < 175)
		{
			motor(leftside)=127; //drive to zone and hit dan
		}
		startTask(Quadreset);
		motor(leftside)=0;
		motor(rightside)=0;
		motor(top)=90;
		wait(.7);
		motor(mobile)=-127;// drop off mobile phone into best friend Rhea's hand
		wait(.5);
		motor(top)=0;
		motor(mobile)=0;
...
//	wait(5); for testing
		...
		startTask(Quadreset);
		while(SensorValue[Quad] < 175 || SensorValue[Quad2] < 300)
		{
			motor(rightside)=127;
