...
void autonomousexe() ////Autonomous is in a void to ease calling it to action
{
	...
	{///////////ENCODERSTUFF
		...
		if(SensorValue(in8)>4000)///////If the arm is LOWER than
		{
			motor[Leftarm] = -20;
	...
	motor[Leftarm] = 10; //////This lifts arm so arm does not immediantly hit floor//// //bottom left
	motor[Rightarm] = -10;

	wait(0.05);


	motor(Leftside) = 127;///jerk to drop claw
	motor(Rightside) = 127;
	wait(0.05);


	motor(Leftside) = -127;//////////JERK/////
	motor(Rightside) = -127;
	wait(0.05);
