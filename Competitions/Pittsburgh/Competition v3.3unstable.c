...
void pre_auton() // Resets encoders
{
	SensorValue[rightEncoder] = 0;
	SensorValue[leftEncoder] = 0;
	SensorValue[solenoid] = 0;
}
task resetEverything() {  // Resets motors and encoders
	motor[DriveLeft_1] = 0;
	motor[DriveLeft_2] = 0;
...
task resetMotors() {  // Resets motors
	motor[DriveLeft_1] = 0;
	motor[DriveLeft_2] = 0;
	motor[DriveRight_1] = 0;
	motor[DriveRight_2] = 0;
}
task autonomous() {  // Start Autonomous
	startTask(resetEverything);
	SensorValue[solenoid] = 0;  // Lowers Pneumatic lift
	motor[armAngle] = 127;
	wait(1);
	motor[armAngle] = 0;
	while(SensorValue[rightEncoder] > -230) {  // Wait until encoder reaches -230
		motor[DriveRight_1] = 63;  // Half speed
	...
	while(SensorValue[rightEncoder] >-900 || SensorValue[leftEncoder] < 900) {
		motor[DriveLeft_1] = 127;  // Full speed
		motor[DriveLeft_2] = 127;
		motor[DriveRight_1] = 127;
		motor[DriveRight_2] = 127;
	}
	startTask(resetMotors);
	SensorValue[solenoid] = 1;  // Hopefully lift mobile goal
	wait1Msec(1000);  // Wait for mobile goal to stop moving


	SensorValue[rightEncoder] = 0;  // Robot then backs out of mobile goal area
	SensorValue[leftEncoder] = 0;
	...
	while(SensorValue[leftEncoder] < 300 || SensorValue[rightEncoder] < 430) {//total shot in the dark
		motor[DriveLeft_1] = 127;  // Rotate 180 at full speed
		motor[DriveLeft_2] = 127;
		motor[DriveRight_1] = -127;
		motor[DriveRight_2] = -127;
	}
...
while(true) {
		...
		if(vexRT[Btn5U] == 1) { //if the left shoulder button is enabled
			setServo(servoGrab, 53); //set the servo's position to 127 (this may be changed based on required clearance)
		}
